...
const val CAMERA_VIDEO_PORT = 5337

/**
 * Streams the encoded H.264 camera feed to one Mac viewer over a raw TCP socket.
 * Identical wire format as [ScreenServer]: length-prefixed Annex-B frames.
 *
 *   [4-byte BE length][1-byte flags: bit0=config(SPS/PPS), bit1=keyframe][Annex-B payload]
 *
 * The latest SPS/PPS and last keyframe are replayed to a freshly-connected viewer so it can
 * start decoding immediately.
 */
class CameraServer(val port: Int = CAMERA_VIDEO_PORT) {
    ...
    private val configFrame = AtomicReference<ByteArray?>(null)
    ...
    fun start() {
        ...
        }.apply { isDaemon = true; name = "TetherCamSrv"; start() }
        Log.i(TAG, "camera video server on $port")
    }
    ...
    private companion object { const val TAG = "TetherCamSrv" }
}
