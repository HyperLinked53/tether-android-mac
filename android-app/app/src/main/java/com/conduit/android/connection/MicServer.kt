...
const val CAMERA_MIC_PORT = 5338

/**
 * Streams raw PCM (48 kHz, mono, 16-bit LE) captured from the phone's microphone to the Mac
 * over a raw TCP socket. Same pattern as [ScreenAudioServer] — no framing, continuous byte stream.
 */
class MicServer(val port: Int = CAMERA_MIC_PORT) {
    ...
    fun start() {
        ...
        Thread {
            while (!ss.isClosed) {
                ...
                runCatching { handleViewer(socket) }.onFailure { Log.e(TAG, "mic viewer error", it) }
            }
        }.apply { isDaemon = true; name = "TetherMicSrv"; start() }
        Log.i(TAG, "mic server on $port")
    }
    ...
    private companion object { const val TAG = "TetherMicSrv" }
}
